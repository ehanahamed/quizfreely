...
if (location.pathname.includes("studyset") === true) {
  let urlParams = new URLSearchParams(document.location
    .search);
  ...
  supabaseClient
    ...
    .then(function(result) {
      ...
      studySet.open();
      document.getElementById("mainActionsSave").classList
        .add("hide");
      supabaseClient.auth.getSession().then(function(
        ...
        if (result.data.session === null) {
          ...
          document.getElementById(
              "mainOpenCreatorNickname").innerText =
            thisStudySet.user_nickname;
        } else if (result.data.session !== null) {
          if (thisStudySet.user_id == result.data
            .session.user.id) {
            states.isSetMine(true);
          } else if (thisStudySet.user_id != result
            .data.session.user
            .id) {
            states.isSetMine(false);
            document.getElementById(
