...
var edit = {
  ...
  load: function(noTitle) {
    edit.makeTableFromArray(
      ...
      document.getElementById(ui.elements.inputs.edit
        .table)
    );
    if (noTitle !== true) {
      document.getElementById(ui.elements.inputs.edit
  ...
  save: function() {
    if (document.getElementById(ui.elements.inputs.edit
        .name).value) {
      sessionData.studySetData.name = document
        .getElementById(
    ...
    sessionData.studySetData.data = edit
      .makeArrayFromTable(
        document.getElementById(ui.elements.inputs.edit
          .table)
      );
  ...
  move: function(index, newIndex) {
    ...
    if (
      ...
      document.getElementById(ui.elements.inputs.edit
          .table).rows[
          newIndex
        ].children[0].children[0].value = document
        .getElementById(
          ui.elements.inputs.edit.table
        ).rows[newOldIndex].children[0].children[0]
        .value;
      document.getElementById(ui.elements.inputs.edit
          .table).rows[
          newIndex
        ].children[1].children[0].value = document
        .getElementById(
          ui.elements.inputs.edit.table
        ).rows[newOldIndex].children[1].children[0]
        .value;
  ...
  makeTableFromArray: function(array, table) {
    for (var i = 0; i < array.length; i++) {
      ...
      table.rows[i + 1].children[0].children[0].value =
        row[0];
      table.rows[i + 1].children[1].children[0].value =
        row[1];
    }
  },
};
