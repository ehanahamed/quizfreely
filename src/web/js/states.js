  ...
  newUser: function(isNewUser) {
    ...
    if (isNewUser === true) {
      if (ui.elements.states.newUser) {
        for (var i = 0; i < ui.elements.states.newUser
          .length; i++) {
          document
            .getElementById(ui.elements.states.newUser[
              i])
            .classList.remove("hide");
        }
      }
      if (ui.elements.states.user) {
        for (var i = 0; i < ui.elements.states.user
          .length; i++) {
          document
  ...
  isSetMine: function(isSetMine) {
    ...
    if (isSetMine === true) {
      if (ui.elements.states.setIsMine) {
        for (var i = 0; i < ui.elements.states.setIsMine
          .length; i++) {
          document
            .getElementById(ui.elements.states
              .setIsMine[i])
            .classList.remove("hide");
        }
      }
      if (ui.elements.states.setIsNotMine) {
        for (var i = 0; i < ui.elements.states
          .setIsNotMine.length; i++) {
          document
            .getElementById(ui.elements.states
              .setIsNotMine[i])
            .classList.add("hide");
